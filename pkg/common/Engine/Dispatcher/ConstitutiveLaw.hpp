...
class ConstitutiveLaw: public Functor2D <
		void, TYPELIST_4(shared_ptr<InteractionGeometry>&, shared_ptr<InteractionPhysics>&, Interaction*, World*)
	>{
	public:
		ConstitutiveLaw(){}
	...
	void addForce (const body_id_t id, const Vector3r& f,World* rb){rb->bex.addForce (id,f);}
	void addTorque(const body_id_t id, const Vector3r& t,World* rb){rb->bex.addTorque(id,t);}
	...
	void applyForceAtContactPoint(const Vector3r& force, const Vector3r& contactPoint, const body_id_t id1, const Vector3r& pos1, const body_id_t id2, const Vector3r& pos2, World* rb){
