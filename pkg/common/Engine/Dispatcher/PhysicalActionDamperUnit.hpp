...
class PhysicalActionDamperUnit: public Functor1D<void,TYPELIST_3(const shared_ptr<PhysicalParameters>&,const Body*, World*)>{
	...
	Vector3r getForceUnsynced (body_id_t id, World* rb){ return rb->bex.getForceUnsynced (id);}
	Vector3r getTorqueUnsynced(body_id_t id, World* rb){ return rb->bex.getTorqueUnsynced(id);}
};
