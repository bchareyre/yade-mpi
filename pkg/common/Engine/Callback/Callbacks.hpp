...
class IntrCallback: public Serializable{
	...
	// should be set at every step by InteractionDispatchers
	Scene* scene;
	/*
	...
	*/
	...
	YADE_CLASS_BASE_DOC(IntrCallback,Serializable,"Abstract callback object which will be called for every (real) :yref:`Interaction` after the interaction has been processed by :yref:`InteractionDispatchers`.\n\nAt the beginning of the interaction loop, ``stepInit`` is called, initializing the object; it returns either ``NULL`` (to deactivate the callback during this time step) or pointer to function, which will then be passed (1) pointer to the callback object itself and (2) pointer to :yref:`Interaction`.\n\n.. note::\n\t(NOT YET DONE) This functionality is accessible from python by passing 4th argument to :yref:`InteractionDispatchers` constructor, or by appending the callback object to :yref:`InteractionDispatchers::callbacks`.\n");
};
