...
void Ip2_ElastMat_ElastMat_LubricationPhys::go(const shared_ptr<Material> &mat1, const shared_ptr<Material> &mat2, const shared_ptr<Interaction> &interaction)
{
    if (interaction->phys)
        return;
...
//    if(otherPhysFunctor)
//    {
//        otherPhysFunctor->go(material1,mat2,interaction);
...
//    {
//            bool ret = otherLawFunctor->go(iGeom,phys->otherPhys,interaction);
...
//    }
    ...
    // Put viscosity for time step:
    phys->cn = pi*phys->eta*3./2.*a*a/h;
    phys->cs = pi*phys->eta/2.*(-2.*a+(2.*a+h)*ln((2.*a+h)/h));

    // Forces and torques
    Vector3r FLn = phys->cn*normVel;
    Vector3r FLs = phys->cs*shearVel;
...
bool Law2_ScGeom_ImplicitLubricationPhys::go(shared_ptr<IGeom> &iGeom, shared_ptr<IPhys> &iPhys, Interaction *interaction)
{
    ...
    Real ue = (std::abs(phys->ue - u1) < std::abs(phys->ue-u2)) ? u1 : u2;
    Vector3r Fn = -ue*g*norm;
    ...
    if(contact)
    {
        ...
        Real Feps = std::abs(-k*(u-ue-phys->eps));
    ...
    Vector3r relTwistVelocity = renAngularVelocity.dot(norm)*norm;
    Vector3r relRollVelocity = relAngularVelocity - relTwistVelocity;

    Vector3r Cr  = -pi*phys->eta*a*a*a*(3./2.*ln(a/h)+63./500.*h/a*ln(a/h))*relRollVelocity;
    Vector3r Ct  = -pi*phys->eta*a*h*ln(a/h)*relTwistVelocity;
