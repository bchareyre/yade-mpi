...
FlowEngine::FlowEngine() : gravity ( Vector3r::ZERO ), isActivated ( true )
{
	...
	currentTes = 0;
	P_zero=10;
	Pressures_Initialized = false;
	PermuteInterval = 1000000000;
...
// 		previous_state = triaxialCompressionEngine->previousState;
...
// 			{
// 				NewTriangulation ( ncb,  flow->T[currentTes],  t2, currentTes );
// 			}

			///Compute flow and and forces here

			Pressures_Initialized = true;
		...
		else if ( current_state==3 )
		{
			...
			cout << y << " pressures initialised -----------------" << endl;
		}
	}
}
...
// 		cout << "position sphere [" << id << "] = " << x << ", " << y << ", " << z << endl;
	...
	cout << contator << "spheres inserted " << endl << endl << "Tesselating------" << endl << endl;
...
// void FlowEngine::NewTriangulation ( Scene* ncb, Tesselation& flow->T[currentTes], Tesselation& t2, int currentTes )
// {
// 	double x_min = 1000.0, x_max = 0.0, y_min = 1000.0, y_max = 0.0, z_min = 1000.0, z_max = 0.0/*, Rmoy=0*/;
//
// 	t2.Clear();
//
// 	FOREACH ( const shared_ptr<Body>& b, *ncb->bodies )
...
// 		{
// 			const InteractingSphere* s = YADE_CAST<InteractingSphere*> ( b->interactingGeometry.get() );
// 			const body_id_t& id=b->getId();
//
// 			Real rad = s->radius;
//
...
// 			Real z = b->state->pos[2];
//
// 			x_min = min ( x_min,x-rad );
// 			x_max = max ( x_max,x+rad );
// 			y_min = min ( y_min,y-rad );
// 			y_max = max ( y_max,y+rad );
// 			z_min = min ( z_min,z-rad );
// 			z_max = max ( z_max,z+rad );
//
//
// 			t2.insert ( x,y,z,rad,id );
// 		}
// 	}
//
// 	flow->AddBoundingPlanes ( t2, x_min, x_max, z_min, z_max, y_max, y_min );
//
// 	flow->Localize ( t2.Triangulation() ); ///*BOUNDARY CONDITIONS*///
//
// 	flow->Compute_Permeability ( t2.Triangulation() );
//
// // 	flow->Sample_Permeability ( t2.Triangulation(), x_min, x_max, z_min, z_max, y_max, y_min );
//
// 	flow->Interpolate ( flow->T[currentTes], t2 );
//
