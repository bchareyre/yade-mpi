...
void NewtonsDampedLaw::handleClumpMemberAccel(MetaBody* ncb, const body_id_t memberId, State* clumpState){
...
void NewtonsDampedLaw::handleClumpMemberAngAccel(MetaBody* ncb, const body_id_t memberId, State* clumpState){
	const shared_ptr<Body>& b=Body::byId(memberId,ncb);
	assert(b->isClumpMember());
	State* state=b->state.get();
...
void NewtonsDampedLaw::handleClumpMemberTorque(MetaBody* ncb, const body_id_t memberId, State* clumpState, Vector3r& M){
...
void NewtonsDampedLaw::saveMaximaVelocity(MetaBody* ncb, const body_id_t id, State* state){
			...
			if (b->isStandalone()){
				...
				if (exactAsphericalRot){
					...
					FOREACH(Clump::memberMap::value_type mm, static_cast<Clump*>(b.get())->members){
						handleClumpMemberAccel(ncb,mm.first,state);
						handleClumpMemberTorque(ncb,mm.first,state,M);
						saveMaximaVelocity(ncb,mm.first,state);
					}
					// motion
					leapfrogTranslate(ncb,state,id,dt);
					leapfrogAsphericalRotate(ncb,state,id,dt,M);
					...
					FOREACH(Clump::memberMap::value_type mm, static_cast<Clump*>(b.get())->members){
						handleClumpMemberAccel(ncb,mm.first,state);
						handleClumpMemberAngAccel(ncb,mm.first,state);
						saveMaximaVelocity(ncb,mm.first,state);
					}
					// forces applied to clump proper, if there are such
...
inline void NewtonsDampedLaw::leapfrogTranslate(MetaBody* ncb, State* state, const body_id_t& id, const Real& dt )
{
	blockTranslateDOFs(state->blockedDOFs, state->accel);
	state->vel+=dt*state->accel;
	state->pos += state->vel*dt + ncb->bex.getMove(id);
}
inline void NewtonsDampedLaw::leapfrogSphericalRotate(MetaBody* ncb, State* state, const body_id_t& id, const Real& dt )
{
...
void NewtonsDampedLaw::leapfrogAsphericalRotate(MetaBody* ncb, State* state, const body_id_t& id, const Real& dt, const Vector3r& M){
