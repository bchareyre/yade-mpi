...
def collectTimingMPI():
	...
	f.write("%s %s %s %g %g %g %g %g %g %g %g\n" % (numThreads, str(os.getenv('OMPI_COMM_WORLD_SIZE')), os.getenv('OMP_NUM_THREADS'), len(O.bodies), O.forces.f(WALL_ID)[1], timing.runtime()/1e9, isendRecvForcesRunner.execTime/1e9, sendRecvStatesRunner.execTime/1e9,waitForcesRunner.execTime/1e9, collisionChecker.execTime/1e9, collider.execTime/1e9))
	f.close()
