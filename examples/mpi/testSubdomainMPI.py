...
def integrate(subD):
	...
	if subD==None:
		...
		mprint ( "MASTER TERMINATED WITH TOTAL FORCE ON FLOOR="+str(O.forces.f(WALL_ID)[1]))
	...
	############## workers ################
	#____0. run
	wprint ( "worker"+str(O._sceneObj.subdomain)+" O.run(1,1)")
	O.run(1,1)
...
if rank == 0: #this is the master process
	...
	for i in range(4): #NxM spheres
		for j in range(2):
			yade.Omega().bodies.append(sphere((i+j/3.,j,0),0.51))
	WALL_ID=O.bodies.append(box(center=(0,-0.5,0),extents=(100,0,100),fixed=True))
	
	##### INITIALIZE MPI #########
	
	#define a subdomain
	sub1=[0,1,2,3] #we send those to subdomain 1
