...
'''
readParamsFromTable(noTableOk=True,N=100,M=100,tiles=5,merge=1) # no.spheres = M*N*tiles
...
if rank is not None: #mpiexec was used
	...
	numThreads=int(sys.argv[1]) # if len(sys.argv)<4 else (int(sys.argv[3]))
...
if rank==0:
	...
	globalDomain = dd.Globaldomain(mn,mx,xDecomp=numThreads-1,yDecomp=1,zDecomp=1) # manually decompose the domain in z direction for given number of threads. Ok choice for using random numbers of MPI threads
	# sequential grain colors
#	import colorsys
#	colorScale = (Vector3(colorsys.hsv_to_rgb(value*1.0/numThreads, 1, 1)) for value in range(0, numThreads))
	colors = []
...
def collectTimingMPI():
	created = os.path.isfile("EngineTimings_2D_strongScalability-POSTHACK-"+timeStr+".dat")
	f=open("EngineTimings_2D_strongScalability-POSTHACK-"+timeStr+".dat",'a')
	if not created: f.write("numThreads mpi omp Nspheres force TotalRuntime isendRecvForces sendRecvStates waitForces collisionChecker collider Merge\n")
	...
	f.write("%s %s %s %g %g %g %g %g %g %g %g %g\n" % (numThreads, str(os.getenv('OMPI_COMM_WORLD_SIZE')), os.getenv('OMP_NUM_THREADS'), len(O.bodies), O.forces.f(WALL_ID)[1], timing.runtime()/1e9, isendRecvForcesRunner.execTime/1e9, sendRecvStatesRunner.execTime/1e9,waitForcesRunner.execTime/1e9, collisionChecker.execTime/1e9, collider.execTime/1e9, merge))
	f.close()
...
if rank==0:
	#end = time.time()
	wallTime = end - start
	created = os.path.isfile("wallTimes_2D_strongScalability-POSTHACK-"+timeStr+".dat")
	f=open('wallTimes_2D_strongScalability-POSTHACK-'+timeStr+'.dat','a')
	if not created: f.write("numThreads mpi omp Nspheres wallTime merge\n")
	f.write("%g %s %s %g %g %g\n" % (numThreads, str(os.getenv('OMPI_COMM_WORLD_SIZE')), os.getenv('OMP_NUM_THREADS'), len(O.bodies),wallTime, merge))
	f.close() 
