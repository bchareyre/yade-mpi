...
mn,mx=Vector3(-1,-15,-100),Vector3(1,15,100) # 2D example
...
mn,mx=Vector3(-1,-15,-60),Vector3(1,10,60)  # for 2D 
...
def collectTimingMPI():
	...
	if not created: f.write("numThreads mpi omp Nspheres force TotalRuntime isendRecvForces sendRecvStates waitForces collisionChecker\n")
	...
	f.write("%s %s %s %g %g %g %g %g %g %g\n" % (numThreads, str(os.getenv('OMPI_COMM_WORLD_SIZE')), os.getenv('OMP_NUM_THREADS'), len(O.bodies), O.forces.f(WALL_ID)[1], timing.runtime()/1e6, isendRecvForcesRunner.execTime/1e6, sendRecvStatesRunner.execTime/1e6,waitForcesRunner.execTime/1e6, collisionChecker.execTime/1e6))
	f.close()
