...
opts.AddVariables(
	...
	ListVariable('features','Optional features that are turned on','opengl,gts,openmp,vtk,qt4',names=['opengl','cgal','openmp','gts','gl2ps','qt4']),
...
if not env.GetOption('clean'):
	...
	if 'cgal' in env['features']:
		...
		env.Append(LIBS=['gmpxx','gmp']) #need to be linked explicitly in ubuntu 11.04 (why not in 10.04?)
