...
void XMLManager::serializeSmartPointer(ostream& stream, Archive& ac , int depth)
{
	...
	{	
		...
		{
			shared_ptr<Serializable> s = ClassFactory::instance().createShared(tmpAc->getSerializableClassName());
			stream << " className=\"" << s->getClassName() << "\" >" << endl;
			writeOpeningTag(stream,*tmpAc,depth);
			tmpAc->serialize(stream, *tmpAc, depth+1);
			writeClosingTag(stream,*tmpAc,depth);
...
{
	shared_ptr<Serializable> s = ClassFactory::instance().createShared(ac.getSerializableClassName());
...
{
	shared_ptr<Serializable> ss = ClassFactory::instance().createShared(ac.getSerializableClassName());
