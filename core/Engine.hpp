...
class Engine: public Serializable{
	...
	YADE_CLASS_BASE_DOC_ATTRS_CTOR_PY(Engine,Serializable,"Basic execution unit of simulation, called from the simulation loop (O.engines)",
		...
		((int, ompThreads, -1,,"Number of threads to be used in the engine. If not specified, the default OMP_NUM_THREADS or the number N defined by 'yade -jN' will be used. This will only affect engines whose code includes an openMP parallel region (e.g. :yref:`InteractionLoop`). This attribute is mostly usefull for experiments or when combining :yref:`ParallelEngine` with engines that run parallel regions, resulting in nested OMP loops with different number of threads at each level."))
