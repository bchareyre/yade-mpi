...
def splitScene():
	...
	if (not O.splittedOnce or REASSIGN_SUBDOMAINS):
          if BISECTION_DECOMPOSITION:                          
            body_list=[] 
            part_list = [] 
            gather_list = []             
            if rank==0: # master sends positions and ids to rank1
                body_list = [(b.state.pos, b.id) for b in O.bodies if type(b.shape)==Sphere]
                comm.send(body_list, dest=1, tag=10) 
            if rank==1: #rank 1 recieves
                body_list = comm.recv(source=0, tag=10) 
            if rank != 0: # all other ranks except master participates in the decomposition
                dec = decomp_bodies(ndim=NDIM)
                body_list = dec.scatter_data(body_list) # each proc gets their list of bodies (ids and pos)
                part_list = [(x[1], rank) for x in body_list]
                gather_list = dec.gather_data(part_list) # rank 1 gathers the ids of the respective subdomains/procs--> (id, rank)
            if rank==1:
                comm.send(gather_list, dest=0, tag=10) #send to master
            if rank==0:                 
