...
def receiveForces(subdomains):
	...
	if 0: #non-blocking:
		...
		for sd in subdomains:
			...
			wprint( "master got forces from "+str(sd)+": "+str(forces))
			for ft in forces:
				#wprint(  "adding force "+str(ft[1])+" to body "+str(ft[0]))
				O.forces.addF(ft[0],ft[1])
				O.forces.addT(ft[0],ft[2])
...
def checkColliderActivated():
	...
	for sd in range(numThreads):
		if sd==rank: needsCollide = typedEngine("InsertionSortCollider").isActivated()
		else: needsCollide=None
		needsCollide=comm.bcast(needsCollide,root=sd) #collective communication
		if needsCollide:
			mprint("rank "+str(rank)+" triggers collider at iter "+str(O.iter) +": paused")
			return True
	return False
...
def isendRecvForces():
	...
	if ACCUMULATE_FORCES:
		if rank!=0:
			...
			print "O.bodies[300].bound", O.bodies[300].bounded #,O.bodies[300].bound.min, O.bodies[300].bound.max
		else: #master
